                ...
                <div className="bg-slate-900/50 p-4 rounded-lg border border-slate-700">
                    ...
                    <div className="flex items-center">
                        <select 
                            ...
                            className="w-full md:w-1/2 px-4 py-2 bg-slate-800 border border-slate-600 text-slate-100 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-50"
                        >
                                    ...
                                    <div key={date}>
                                        ...
                                        <div className="space-y-2 pl-4 border-l-2 border-slate-700">
                                            {matchesByStageAndDate[stage as MatchStage][date].map(match => {
                                                ...
                                                return (
                                                    <div key={match.id} className={`p-3 rounded-md space-y-2 ${isDisabled ? 'bg-slate-800/50' : 'bg-slate-900/50'}`}>
                                                        <div className="grid grid-cols-12 gap-2 items-center text-sm">
                                                            ...
                                                            <div className="col-span-2 flex items-center justify-center gap-1">
                                                                <input type="number" min="0" value={pred?.team1Score > -1 ? pred.team1Score : ''} onChange={e => handleScoreChange(match.id, 'team1Score', e.target.value)} disabled={isDisabled} className="w-10 text-center bg-slate-800 border border-slate-600 text-white font-bold rounded-md disabled:opacity-50" />
                                                                <span className="text-slate-500">-</span>
                                                                <input type="number" min="0" value={pred?.team2Score > -1 ? pred.team2Score : ''} onChange={e => handleScoreChange(match.id, 'team2Score', e.target.value)} disabled={isDisabled} className="w-10 text-center bg-slate-800 border border-slate-600 text-white font-bold rounded-md disabled:opacity-50" />
                                                            </div>
