...
const InviteModal = ({ tournament, onClose, onParticipantsChange }: InviteModalProps) => {
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-70 flex justify-center items-center z-50 p-4">
            <div className="bg-slate-800 border border-slate-700 p-6 rounded-lg shadow-xl max-w-4xl w-full">
                ...
                <input
                    ...
                    className="w-full px-4 py-2 mb-4 bg-slate-900 border border-slate-600 text-slate-100 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                />
                        ...
                        <div className="space-y-2">
                            ...
                            <ul className="space-y-2 pr-2">
                                {availableUsers.map(user => (
                                    <li key={user.uid} className="flex justify-between items-center p-2 bg-slate-700/50 rounded-md">
                                        <div>
                                            <p className="font-medium text-white">{user.name}</p>
                                            <p className="text-xs text-slate-400">{user.email}</p>
                                        </div>
                                        <button onClick={() => handleInvite(user.uid)} className="px-3 py-1 bg-green-600 hover:bg-green-500 text-white text-xs font-semibold rounded-md">Invite</button>
                                    </li>
                                ))}
                            </ul>
                        </div>
                        ...
                        <div className="space-y-2">
                            ...
                            <ul className="space-y-2 pr-2">
                                {joinedUsers.map(user => (
                                    <li key={user.uid} className="flex justify-between items-center p-2 bg-slate-900 rounded-md">
                                        <div>
                                            <p className="font-medium text-white">{user.name}</p>
                                            <p className="text-xs text-slate-400">{user.email}</p>
                                        </div>
                                        <button onClick={() => handleRemove(user.uid)} className="px-3 py-1 bg-red-800 hover:bg-red-700 text-white text-xs font-semibold rounded-md">Remove</button>
                                    </li>
                                ))}
                            </ul>
                        </div>
                ...
                <div className="mt-6 flex justify-end">
                    <button onClick={onClose} className="px-6 py-2 bg-slate-600 hover:bg-slate-500 font-semibold text-white rounded-md">Close</button>
                </div>
            </div>
        </div>
    );
